...
int main()
{
    ...
    for (int j = 0; j < 14; j++)
    {
        if(j == 0) // write to R1
        {
            latches[27].before = 0x15; //input1 rf
            latches[22].before = 1; //input2 rf
            latches[30].before = 0b11; //RF control
        }
        if(j == 1) // write to R2
        {
            latches[27].before = 0x2; //input1 rf
        ...
        if(j==7) {
            latches[34].before = 0;
            latches[30].before = 0b00;
        }
        if(j==8) {
            latches[37].before = 1; //a lovely add for uncle bouaziz please
        }
        if(j==9) {
            latches[37].before = 0; 
            latches[41].before = 1; //Set Data memory to wirte
            latches[38].before = 0x99; //Set Data to be written
        }
        if(j==10) {
            latches[41].before = 0; //Set Data memory to read
            latches[36].before = 0x17; // Hardwire but no worries
        }
        
        std::cout << "time: " << j << " RF 1: " << latches[27].before << std::endl;
        std::cout << "time: " << j << " RF 2: " << latches[22].before << std::endl;
        std::cout << "time: " << j << " RFo 1: " << (int)latches[28].before <<std::endl;
        std::cout << "time: " << j << " RFo 2: " << (int)latches[29].before <<std::endl;
        std::cout << "time: " << j << " ALU 1: " << latches[31].before << std::endl;
        std::cout << "time: " << j << " ALU 2: " << latches[35].before << std::endl;
        std::cout << "time: " << j << " ALUo: " << latches[36].before << std::endl;
        std::cout << "time: " << j << " DMo: " << latches[40].before << std::endl;
    ...
    std::cout << latches[36].before << std::endl;
    std::cout << latches[40].before << std::endl;
