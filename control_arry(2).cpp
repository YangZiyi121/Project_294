...
public:
	...
	ControlArray(Latch input[], Latch output[])
	{
		...
		for (int i = 0; i < arraysize; i++)// salman change size to arraysize
		{
	...
	void receive_clock() {
        string s = to_string(ControlArray::result);
		ControlArray::out[0]->before = convert_bintdec( stoi(s.substr(0, 1)));//PC MAX 1 bit
